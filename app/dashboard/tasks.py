...
from app.dashboard.task_state_controller import get_user_id, get_dialog_key, get_task_info_key, clear_task_dialog_state

# Team members list for @mention matching
TEAM_MEMBERS = ["Marina", "Nick", "Mel", "Charles", "Alexey", "Tony", "Elena", "Vlad", "Slava"]


def cleanup_old_dialog_state():
    """Remove all old dialog and task_info session keys on page load"""
    # Only clean up legacy keys, not the new user-scoped keys
    keys_to_delete = [
        k for k in st.session_state.keys() 
        if k.startswith("show_results_dialog_") or k.startswith("completed_task_info_")
    ]
...
def parse_task_input(input_text: str) -> Tuple[str, Optional[date], Optional[str], Optional[str]]:
    ...
    # Extract assignee (@username) with case-insensitive matching
    assignee_match = re.search(r'@(\w+)', text)
    if assignee_match:
        mentioned_name = assignee_match.group(1)
        # Case-insensitive lookup in team members
        assignee = None
        for team_member in TEAM_MEMBERS:
            if team_member.lower() == mentioned_name.lower():
                assignee = team_member  # Use properly capitalized version
                break
        # If no match found, use the mentioned name as-is (for backward compatibility)
        if assignee is None:
            assignee = mentioned_name
...
def show_task_view_segmented_control(company_id: str, all_tasks: List[Task]):
    """Display segmented control for task view (Open/Completed/All) with counts"""
    # Calculate counts using centralized status functions
    active_count = len([t for t in all_tasks if is_active_status(t.status)])
    completed_count = len([t for t in all_tasks if is_completed_status(t.status)])
    ...
    # Native segmented control (requires Streamlit >= 1.49.0)
    selected = st.segmented_control(
        label="Filter tasks by status",
        options=options,
        default=f"Active Tasks ({active_count})",
        label_visibility="collapsed",
        key=f"task_segment_{company_id}"
    )
    
    # Update session state
    if selected and "Active Tasks" in selected:
        st.session_state[f"task_view_{company_id}"] = "Open"
    elif selected and "Completed" in selected:
        st.session_state[f"task_view_{company_id}"] = "Completed"
    elif selected and "All" in selected:
        st.session_state[f"task_view_{company_id}"] = "All"
    else:
        # Default to "Open" if selected is None or doesn't match expected patterns
        st.session_state[f"task_view_{company_id}"] = "Open"


def show_tasks_section(company):
    """Main tasks section for company pages with data editor"""
    # Clean up old dialog state on page load
    cleanup_old_dialog_state()
    
    # Apply compact Linear-style CSS (without tab modifications)
    st.markdown("""
    <style>
    ...
    # Single line: Task input + Add button
    show_add_task_form_with_button(company_id)
    
    # Single line: Segmented controls (left) + Filter dropdown (right) - smaller filter
    col_segments, col_filter = st.columns([3, 1])
    
    with col_segments:
        show_task_view_segmented_control(company_id, all_tasks)
    ...
    with col_filter:
        filter_option = st.selectbox(
            ...
            label_visibility="collapsed",
            key=f"task_filter_view_{company_id}",
            index=0
        )
    
    # Apply segment filter first (by status: active/completed/all)
    segment_key = f"task_view_{company_id}"
    selected_segment = st.session_state.get(segment_key, "Open")
    
    if selected_segment == "Open":
        filtered_tasks = [t for t in all_tasks if is_active_status(t.status)]
    elif selected_segment == "Completed":
        filtered_tasks = [t for t in all_tasks if is_completed_status(t.status)]
    else:  # "All"
        filtered_tasks = all_tasks
    
    # Apply dropdown filter on already filtered tasks
    today = date.today()
    current_user = get_current_user_for_tasks()
    
    if filter_option == "All tasks":
        # No additional filtering - keep current filtered_tasks
        pass
    elif filter_option == "My active tasks":
        if current_user:
            # Only apply this filter if we're in "Open" or "All" segment
            if selected_segment in ["Open", "All"]:
                filtered_tasks = [t for t in filtered_tasks if is_active_status(t.status) and t.assignee and current_user.lower() in t.assignee.lower()]
            else:
                filtered_tasks = []  # No active tasks in "Completed" segment
        else:
            filtered_tasks = []
    elif filter_option == "Created by me":
        if current_user:
            filtered_tasks = [t for t in filtered_tasks if t.created_by and current_user.lower() in t.created_by.lower()]
        else:
            filtered_tasks = []
    elif filter_option == "Overdue":
        # Only apply this filter if we're in "Open" or "All" segment
        if selected_segment in ["Open", "All"]:
            filtered_tasks = [t for t in filtered_tasks if is_active_status(t.status) and t.due_date and t.due_date < today]
        else:
            filtered_tasks = []  # No overdue tasks in "Completed" segment
    elif filter_option == "Due today":
        # Only apply this filter if we're in "Open" or "All" segment
        if selected_segment in ["Open", "All"]:
            filtered_tasks = [t for t in filtered_tasks if is_active_status(t.status) and t.due_date and t.due_date == today]
        else:
            filtered_tasks = []  # No due today tasks in "Completed" segment
    elif filter_option == "Due this week":
        # Only apply this filter if we're in "Open" or "All" segment
        if selected_segment in ["Open", "All"]:
            week_end = today + timedelta(days=7)
            filtered_tasks = [t for t in filtered_tasks if is_active_status(t.status) and t.due_date and today <= t.due_date <= week_end]
        else:
            filtered_tasks = []  # No due this week tasks in "Completed" segment
    
    # Add debug panel to sidebar
    if st.sidebar.checkbox("🐛 Show task debug state", key="debug_tasks"):
        task_state = {
            k: v for k, v in st.session_state.items() 
            if "task" in k.lower() or "dialog" in k.lower()
        }
        st.sidebar.json(task_state)
        
        # Show filtering debug info
        st.sidebar.write("**Filter Debug:**")
        st.sidebar.write(f"Selected segment: {selected_segment}")
        st.sidebar.write(f"Filter option: {filter_option}")
        st.sidebar.write(f"Total tasks: {len(all_tasks)}")
        st.sidebar.write(f"Active tasks: {len([t for t in all_tasks if is_active_status(t.status)])}")
        st.sidebar.write(f"Completed tasks: {len([t for t in all_tasks if is_completed_status(t.status)])}")
        st.sidebar.write(f"Filtered tasks: {len(filtered_tasks)}")
    
    # Show tasks in data editor based on selected segment
    if filtered_tasks:
        show_tasks_data_editor(filtered_tasks, company_id)
    else:
        if selected_segment == "Open":
            st.info("No active tasks yet. Add the first follow-up.")
        elif selected_segment == "Completed":
...
def show_tasks_data_editor(tasks: List[Task], company_id: str):
    """Display tasks using st.data_editor with inline editing based on segment selection"""
    # Get the selected segment to determine display mode
    segment_key = f"task_view_{company_id}"
    selected_segment = st.session_state.get(segment_key, "Open")
    
    # Filter tasks based on segment selection
    if selected_segment == "Open":
        display_tasks = [t for t in tasks if is_active_status(t.status)]
    elif selected_segment == "Completed":
        display_tasks = [t for t in tasks if is_completed_status(t.status)]
    else:  # "All"
        display_tasks = tasks
    
    # Prepare DataFrame for display tasks
    df = prepare_tasks_dataframe(display_tasks)
    
    # Handle dialog state management with scoped keys
    user_id = get_user_id()
    
    # Top-level dialog container - renders BEFORE empty check to handle last task completion
    with st.container():
        # Check for and show any pending results dialogs using scoped keys
        dialog_task_id = None
        for task in tasks:  # Check all tasks, not just display_tasks
            dialog_key = get_dialog_key(task.id, user_id)
            if st.session_state.get(dialog_key, False):
                dialog_task_id = task.id
                task_info_key = get_task_info_key(task.id, user_id)
                
                # Debug: Show dialog keys in sidebar if debug is enabled
                if st.session_state.get("debug_tasks", False):
                    st.sidebar.write(f"🔍 Dialog key: {dialog_key}")
                    st.sidebar.write(f"🔍 Task info key: {task_info_key}")
                    st.sidebar.write(f"🔍 Task ID: {task.id}")
                ...
                if task_info_key in st.session_state:
                    ...
                    temp_task = Task(
                        ...
                        result_history=task_info.get('result_history', [])  # Include history
                    )
                    show_task_results_dialog(temp_task)
                    # Don't clear dialog state here - let dialog handle its own cleanup
    ...
    team_members = ["Unassigned"] + TEAM_MEMBERS
    
    # Configure columns based on segment selection
    if selected_segment == "Completed":
        # For completed tasks, show different columns
        column_config = {
        ...
        # For active and all tasks, show standard columns
        column_config = {
            "id": None,  # Hide ID column
            "status": None,  # Hide status column
            "created_at": None,  # Hide created_at column
    ...
    session_key = f"tasks_df_{company_id}_{selected_segment.lower()}"
    if session_key not in st.session_state or len(st.session_state[session_key]) != len(df):
        st.session_state[session_key] = df.copy()
    
    # Set disabled columns based on segment
    if selected_segment == "Completed":
        disabled_columns = ["id", "status", "created_at"]
    else:
    ...
    edited_df = st.data_editor(
        ...
        key=f"tasks_editor_{company_id}_{selected_segment.lower()}"
    )
    
    # Handle edits - only process if there are actual changes
    if not edited_df.equals(st.session_state[session_key]):
        if selected_segment == "Completed":
            handle_completed_task_edits(edited_df, st.session_state[session_key], company_id, display_tasks)
        else:
            handle_task_edits(edited_df, st.session_state[session_key], company_id, display_tasks)
...
def handle_task_edits(edited_df: pd.DataFrame, original_df: pd.DataFrame, company_id: str, all_tasks: List[Task]):
    ...
    for idx in range(len(edited_df)):
        ...
        if edited_completed != orig_completed:
            if edited_completed:
                ...
                if st.session_state.get("allow_quick_complete", False):
                    ...
                    # Store task info in session state for dialog using scoped keys
                    ...
                    if task:
                        ...
                        # Atomic session state update for dialog
                        session_updates = {
                            task_info_key: {
                                'id': task.id,
                                ...
                                'result_history': task.result_history if hasattr(task, 'result_history') else []
                            },
                            dialog_key: True
                        }
        ...
        # Apply all updates for this task at once with error handling
        if task_updates:
            try:
                update_task(task_id, **task_updates)
                changes_made = True
                
                # CRITICAL FIX: Force immediate rerun after status changes
                # This ensures the task list is refreshed with the new status
                if 'status' in task_updates:
                    # Optional debounce for rapid updates
                    time.sleep(0.05)
                    st.rerun()
            except Exception as e:
...
def handle_completed_task_edits(edited_df: pd.DataFrame, original_df: pd.DataFrame, company_id: str, all_tasks: List[Task]):
    ...
    for idx in range(len(edited_df)):
        ...
        # Get values for comparison
        edited_task = str(edited_df.iloc[idx]['title'])
        orig_task = str(original_df.iloc[idx]['title'])
        edited_owner = str(edited_df.iloc[idx]['owner'])
        orig_owner = str(original_df.iloc[idx]['owner'])
        ...
        if orig_completed and not edited_completed:
            # Store the current result history before reactivating
            current_task = next((t for t in all_tasks if t.id == task_id), None)
            if current_task and current_task.outcome:
                # Store the outcome in session state for later use (backward compatibility)
                st.session_state[f"reactivated_task_outcome_{task_id}"] = current_task.outcome
            
            if current_task and hasattr(current_task, 'result_history'):
                # Store full history in session state
                st.session_state[f"reactivated_task_history_{task_id}"] = current_task.result_history
            ...
            update_task(
                ...
                completed_at=None,
                outcome=None  # Clear current outcome but preserve history
                # result_history stays intact in the data
            )
...
def show_completed_tasks_section(all_tasks: List[Task], company_id: str):
    ...
    completed_tasks = [t for t in all_tasks if t.status == TaskStatus.COMPLETED.value]
...
def show_task_summary_card(company):
    ...
    active_count = len([t for t in tasks if is_active_status(t.status)])
    
    if active_count > 0:
        overdue_tasks = [t for t in tasks if is_active_status(t.status) and t.due_date and t.due_date < date.today()]
...
def show_add_task_form_compact(company_id: str):
    """Compact form to add a new task with enhanced placeholder and no submit button - Enter key only"""
    
    # Initialize session state for the input
    input_key = f"task_input_compact_{company_id}"
    if input_key not in st.session_state:
        st.session_state[input_key] = ""
    
    def handle_task_submit():
        """Handle task submission when Enter is pressed"""
        task_input = st.session_state[input_key]
        if task_input and task_input.strip():
            ...
            if error_msg:
                ...
                try:
                    task = add_task(company_id, title, due_date, assignee, created_by=creator)
                    # Store success message in session state to persist across rerun
                    st.session_state[f"task_added_success_compact_{company_id}"] = title
    ...
    # Create the text input with on_change callback
    st.text_input(
        "Add new task",
        placeholder="Add task e.g., setup call on 10/11 @Nick",
        key=input_key,
        on_change=handle_task_submit,
        label_visibility="collapsed"
    )
    
    # Show success message if task was just added
    if f"task_added_success_compact_{company_id}" in st.session_state:
        st.success(f"✅ Task added: {st.session_state[f'task_added_success_compact_{company_id}']}")
        # Clear the success message after displaying
        del st.session_state[f"task_added_success_compact_{company_id}"]


def show_add_task_form_with_button(company_id: str):
    """Form to add a new task with both Enter key and button submission"""
    with st.form(key=f"add_task_form_with_button_main_{company_id}", clear_on_submit=True):
        ...
        with col_input:
            task_input = st.text_input(
                "Add new task",
                placeholder="Add task e.g., setup call on 10/11 @Nick",
                key=f"task_input_with_button_main_{company_id}",
                label_visibility="collapsed"
            )
        ...
        # Handle form submission (triggered by Enter key or button)
        if submitted and task_input and task_input.strip():
            # Parse the input
            title, due_date, assignee, error_msg = parse_task_input(task_input)
            ...
            if error_msg:
                ...
                try:
                    task = add_task(company_id, title, due_date, assignee, created_by=creator)
                    # Store success message in session state to persist across rerun
                    st.session_state[f"task_added_success_with_button_main_{company_id}"] = title
                    st.rerun()
                except Exception as e:
                    st.error(f"Error adding task: {str(e)}")
            else:
    ...
    # Show success message if task was just added
    if f"task_added_success_with_button_main_{company_id}" in st.session_state:
        st.success(f"✅ Task added: {st.session_state[f'task_added_success_with_button_main_{company_id}']}")
        # Clear the success message after displaying
        del st.session_state[f"task_added_success_with_button_main_{company_id}"]
